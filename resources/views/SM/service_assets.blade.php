...
<div class="card">
    ...
    <div class="card-body">
        <div class="input-group mb-3">
            ...
            <input type="text" class="form-control" name="hint" id="hint"  aria-label="Username" placeholder="Enter Serial/ ML-NOL / CON Code / Customer Name">
          </div>
    </div>
  </div>
</div>
     ...
     if (hint.length >= 4 ) {        
       ...
       $("tbody#table1").html("");
        ...
        $.each(data, function(index, value){
            var Sdate  = new Date(value.udSAContractStart).toLocaleDateString('en-GB');
            var Edate  = new Date(value.udSAContractEnd).toLocaleDateString('en-GB');
 
                
 
         
   
     var html = ("<tr><td>"+value.Name+"</td><td>"+value.ucSASerialNo+ "</td><td>"+value.CDescription+ "</td><td>" +value.cCode+ "</td><td>" +(value.ucSABillingAsset ?? '')+"</td><td>" +Sdate+ "</td><td>" +Edate+ "</td></tr>");
