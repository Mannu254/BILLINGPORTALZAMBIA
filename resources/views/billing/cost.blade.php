  ...
  <script> 
   ...
   $('#hint_con').on('keyup',function() {    
     ...
     if (hint.length >= 4 ){     
      ...
      var url ="<button class='btn btn-xs btn-primary' onClick='ShowModal(this)' data-id="+value.iMeterId+"><i class='fa fa-pencil-square-o'></i></button>";
...
success: function (data) {
  ...
  $.each(data, function(index, value){
    ...
    var html = ("<tr><td>"+'<input type="number" readonly value='+ value.AutoIdx +' id="id" class="qty" name="id[]">'+" </td><td>"+'<input type="number" readonly value='+ value.iFromQty +' id="fromqty" class="fromqty" name="fromqty[]">'+"</td><td>"+'<input type="number" value='+ value.iToqty +' id="toqty" readonly class="toqty" name="toqty[]">'+"</td><td>"+'<input type="number" value='+value.frate+' id="rate" class="rate" name="rate[]">'+"  </td></tr>");
