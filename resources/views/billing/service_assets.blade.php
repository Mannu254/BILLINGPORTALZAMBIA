...
<div class="card">
    ...
    <div class="card-body">
        <div class="input-group mb-3">
            ...
            <input type="text" class="form-control" name="hint" id="hint"  aria-label="Username" placeholder="Enter Serial / CON Code / Customer Name">
          </div>
    </div>
  </div>
</div>
...
<table id="table" class="table table-hover table-bordered table-sm text-center">
  ...
  <script> 
   ...
   $('#hint').on('keyup',function() {    
     ...
     var html = ("<tr><td>"+value.Name+"</td><td>"+value.ucSASerialNo+ "</td><td>"+value.Desc+ "</td><td>" +value.ConCode+ "</td><td>" +(value.ucSABillingAsset ?? '')+"</td><td>" +Sdate+ "</td><td>" +Edate+ "</td></tr>");
