...
class AssetUpdate implements ToCollection,WithStartRow,WithValidation,SkipsEmptyRows,WithCalculatedFormulas
{
    use Importable;
    ...
    */
    ...
    public function collection(Collection  $rows)
    {
        ...
        {  
            ...
            $update =DB::connection('service_manager')->table('_smtblServiceAsset')
        ...
        elseif(!empty($row[0]) && !empty($row[1])){
            ...
            $contract_id =DB::connection('service_manager')->table('_smtblContractMatrix')
            ->where('cCode',$row[1])
            ->select('AutoIdx')
            ->first();
